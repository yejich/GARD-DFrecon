...
def main():
 ...
 # Resize the exact previous input crop, preserving scene content for comparison.
 pair.dist_views=[cv2.resize(a,(504,378),interpolation=cv2.INTER_AREA) for a in pair.dist_views]
 pair.clean_views=[cv2.resize(a,(504,378),interpolation=cv2.INTER_AREA) for a in pair.clean_views]
 pair.image_size_hw=(378,504)
 info['pre_resize_crop_hw']=list(info['target_hw']);info['target_hw']=[378,504]
 info['resize_mode']='Existing 588x896 crop resized with INTER_AREA; no additional crop; aspect ratio changes.'
