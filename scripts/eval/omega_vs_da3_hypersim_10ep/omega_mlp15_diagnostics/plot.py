...
for ax,layer in zip(axs,[15,16,17]):
...
fig.tight_layout(rect=(0,0,1,.9));fig.savefig(P/'blocks15_16_17_comparison.png',dpi=180);plt.close(fig)
(P/'README.md').write_text('Same three groups, checkpoint, seeds, sampler and read-only hooks as omega_mlp17_diagnostics, targeting frame_blocks[15]. Compare stage 0 (frame-attention output / MLP input), stage 5 (LayerScale residual), stage 6 (residual addition). Patch tokens only, no spatial masks. FP32 cosine, BF16 inference. comparison_summary.json stores per-group and mean cosine for blocks 15, 16 and 17. No inference or training settings modified.\n')
