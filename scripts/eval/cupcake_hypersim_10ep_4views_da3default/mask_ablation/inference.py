...
ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=['self_token','self_view'],required=True);mode=ap.parse_args().mode
OUT=output_dir(__file__)/mode
...
def main():
 ...
 meta={'mask_mode':mode,'mask_layer':9,'query_xy':[[175,259],[273,105]],'checkpoint':str(run/'checkpoints/latest.pt'),'epoch':10,'ema':True,'cfg':False,'sampler':OmegaConf.to_container(cfg.sampler),'noise_lvl':.3,'training_lq_cond':True,'seed':42,'time_dist_shift':shift,'input_setting':info,'metrics':metrics}
