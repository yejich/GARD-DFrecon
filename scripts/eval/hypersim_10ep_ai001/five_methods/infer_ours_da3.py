...
@torch.no_grad()
def evaluate(i):
 item=ds[i]; group_dir=OUT/f'group_{i:02d}';group_dir.mkdir(exist_ok=True)
 group=ds.manifest['eval_groups'][i]
 records=[ds.records[j] for j in group['indices']]
 (group_dir/'inputs.json').write_text(json.dumps({'group':i,'records':records,'distractor':item['distractor_views']},indent=2))
 for j,a in enumerate(item['lq_views']):Image.fromarray(a).save(group_dir/f'input_{j}.png')
 for j,a in enumerate(item['hq_views']):Image.fromarray(a).save(group_dir/f'clean_{j}.png')
 np.save(group_dir/'masks.npy',item['pixel_masks'])
 clean=np.array(item['hq_views'])/255.; inp=np.array(item['lq_views'])/255.
 ...
 geo=model._geometry_from_output(base)
 np.savez_compressed(group_dir/'da3.npz',**geo,images=inp.astype(np.float32),decoded_rgb=baseline.astype(np.float32))
 del base,features
 ...
 geo=model._geometry_from_output(restored)
 rgb=decode_rgb_from_feats(rae,restored.feat,H,W)[0].permute(0,2,3,1).numpy(); mask=item['pixel_masks']>0; metrics={}
 np.savez_compressed(group_dir/'ours.npz',**geo,images=inp.astype(np.float32),decoded_rgb=rgb.astype(np.float32))
